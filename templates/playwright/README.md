# [LAB NAME] <!-- TODO: replace with lab name, e.g. "UI Automation Sprint" -->
...
> The environment will take about 60 seconds to set up. Everything is pre-installed — Node.js, Playwright, and GitHub Copilot.
...
```
You push to main
    → GitHub Actions runs Playwright tests on .grader/checks/ + your tests
    → Score is calculated
...
Run `npx playwright install --with-deps chromium` in the terminal to install the browser.

**Tests failing locally but passing in Actions (or vice versa)**
Run `npm install` in the terminal to ensure all dependencies are installed.

**Codespace running out of memory during tests**
Use `npm run grade` instead of `npm test` — it runs only the grader checks without loading full browser sessions.
